...
`monitor-iroh` builds endpoints with `presets::Minimal`. This installs the
...
The application protocol uses ALPN `tray-monitor/metrics/1` and length-delimited
...
`monitor-iroh`; upgrading requires the NAT and privacy matrices in
