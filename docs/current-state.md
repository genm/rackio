...
| Tray | A single running instance (a second launch hands off to the first and opens its dashboard); one status item per machine titled with a one-glyph badge and state symbol — the name's initial, or an icon (one character or emoji) chosen on the machine's card and kept as a desktop preference; one submenu per machine carrying the same metrics as the dashboard card — CPU, memory, swap, the fullest filesystem by mount name, temperature, network rate and uptime — plus the connection path and RTT | Metrics an unreachable machine cannot report show an em dash and no bar rather than a stale value |
