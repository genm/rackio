...
| Health thresholds | Built-in disk capacity rules (90 % warning, 95 % critical, three samples), operator-replaceable per machine, each breach published as a named detail line that reaches the card and the OS notification | CPU, memory and temperature levels stay operator-defined; rules are read at daemon start, so editing them needs a restart |
